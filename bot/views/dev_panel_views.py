...
from bot.views.embed_builder import build_success_embed, build_error_embed, build_info_embed
                ...
                async def on_submit(inner_self, inner_interaction: Interaction):
                    ...
                    if not guild_id_str.isdigit():
                        await inner_interaction.response.send_message(
                            embed=build_error_embed("無效輸入", "伺服器 ID 必須是數字。", bot_user=interaction.client.user), ephemeral=True
                        )
                        return
                    ...
                    if not guild:
                        await inner_interaction.response.send_message(
                            embed=build_error_embed("伺服器不存在", f"找不到 ID 為 {guild_id} 的伺服器，或機器人不在該伺服器中。", bot_user=interaction.client.user), ephemeral=True
                        )
                        return

                    if await is_server_banned(guild_id):
                        await inner_interaction.response.send_message(
                            embed=build_info_embed("已封禁", f"伺服器 `{guild.name}` (ID: {guild_id}) 已處於封禁狀態。", bot_user=interaction.client.user), ephemeral=True
                        )
                        return
                    ...
                    try:
                        ...
                        await inner_interaction.response.send_message(
                            embed=build_error_embed("權限不足", "機器人沒有權限離開伺服器。", bot_user=interaction.client.user), ephemeral=True
                        )
                        logger.error(
                        ...
                        await inner_interaction.response.send_message(
                            embed=build_error_embed("封禁失敗", f"封禁伺服器失敗：{str(e)}", bot_user=interaction.client.user), ephemeral=True
                        )
            ...
            class UnbanServerModal(ui.Modal, title="解除伺服器封禁"):
                ...
                async def on_submit(inner_self, inner_interaction: Interaction):
                    ...
                    if not await is_server_banned(guild_id):
                        await inner_interaction.response.send_message(
                            embed=build_info_embed("未封禁", f"伺服器 ID `{guild_id}` 並未處於封禁狀態。", bot_user=interaction.client.user), ephemeral=True
                        )
                        return
                    ...
                    try:
                        ...
                        await inner_interaction.response.send_message(
                            embed=build_error_embed("解除封禁失敗", f"解除伺服器封禁失敗：{str(e)}", bot_user=interaction.client.user), ephemeral=True
                        )
    ...
    async def callback(self, interaction: Interaction):
        ...
        if not guild_list:
            await interaction.followup.send(
                embed=build_info_embed("無伺服器", "機器人目前沒有加入任何伺服器。", bot_user=interaction.client.user), ephemeral=True
            )
            return
...
class ViewBannedServersButton(ui.Button):
    ...
    async def callback(self, interaction: Interaction):
        ...
        if not banned_servers_data:
            await interaction.followup.send(
                embed=build_info_embed("無封禁伺服器", "目前沒有任何封禁的伺服器。", bot_user=interaction.client.user), ephemeral=True
            )
            return
        ...
        class StatusInputModal(ui.Modal, title="自訂狀態設定"):
            ...
            async def on_submit(inner_self, inner_interaction: Interaction):
                ...
                if activity:
                    ...
                    await inner_interaction.response.send_message(
                        embed=build_success_embed("狀態更新成功", f"狀態已更新為 `{text}`。", bot_user=interaction.client.user), ephemeral=True
                    )
                    logger.debug(f"Bot status updated to {selected}: {text}")
                else:
                    await inner_interaction.response.send_message(
                        embed=build_error_embed("狀態設定失敗", "無法設定此狀態。", bot_user=interaction.client.user), ephemeral=True
                    )
